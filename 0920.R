...
ggplot(data = df1, aes(x=부서, y=매출))+
  ...
  labs(tatle = "팀별 영업실적", x="부서명", y="매출(억원)")
# 각 부서별 매출건수수
ggplot(data = df1, aes(x=부서))+
  geom_bar(stat = "count")+
  labs(tatle = "팀별 영업실적", x="부서명", y="매출건수")
#막대에 색상 자동지정
...
ggplot(df1, aes(x=부서, y=매출))+
  geom_bar(stat="identity", fill="magenta", color="white") +
  labs(title="부서별 영업실적", x="부서명", y="매출(억원")

# 막대마다 사용자가 지정한 색상으로 변경
ggplot(df1, aes(x=부서, y=매출))+
  geom_bar(stat="identity", fill=c("red", "blue","green"), color="black") +
  labs(title="부서별 영업실적", x="부서명", y="매출(억원")

# 막대에 레인보우색상 중 눈에 잘띄는 색상으로 자동 변경
ggplot(df1, aes(x=부서, y=매출))+
